...
export default class BasicBot {
  ...
  public findBed() {
    let b = this.bot.findBlock(
      ...
      (e, bl) => {
        if (bl.length) {
          this.bot.activateBlock(bl[0], (err) => {
            if (err) this.bot.chat(err.toString())
          })
  ...
  parseMessage(username: string, message: string, whisper?: boolean) {
    ...
    if (
      message.toLowerCase().includes(process.env.NICKNAME.toLowerCase()) &&
      isAskingWhereYouAre(message.toLowerCase())
    ) {
      this.reportPosition()
    }
  }
